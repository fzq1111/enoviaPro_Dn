...
proc pPrpocessAdminType { pFileDataAdmin pPerson } {
	...
	foreach lLineDataAdmin $lDataAdmin {
		...
		if { $sPerson == $pPerson } {
			 ...
			 foreach  AdminAccess $pLineDataAdmin column $lHeaderAdminColumn {
				...
				if { $pHasAccess == "y" } {
						set nlengt [string length $pAdminAccess ]
					if { $nlengt >0 && $clength>0 } {
						append pAdminAccess "," $column
					} elseif { $clength>0 } {
						append pAdminAccess $column
					}
					incr nlength
                      ...
                      append sCmd " " "\"$sNameValue\""
                       ...
                       if { $i == "assign_role"} {
						...
						# QHV 21-11-2012 SR00116155 "append Administration Access"
						  if {$i == "type"} {
						  	set sAdminAccess [pPrpocessAdminType $lFileDataAdmin $sNameValue ]
							if { $sAdminAccess != ""} {
