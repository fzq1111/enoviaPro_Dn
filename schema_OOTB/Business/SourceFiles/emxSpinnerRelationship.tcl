...
proc pAnalyzeRelationship {} {
   global aCol aDat bOverlay bAdd sMxVersion lsAttributePlan lsAttributeActual lsFromTypePlan lsFromTypeActual lsFromRelPlan lsFromRelActual lsToTypePlan lsToTypeActual lsToRelPlan lsToRelActual lsSchemaType bMm
...
# Major/Minor Mod - ION - 10/1/2012
   if {$bMm} {set aCol(23) [pCompareAttr $aCol(23) false true true true]}
...
# Major/Minor Mod - ION - 10/1/2012
      if {$bMm} {set aDat(23) [pPrintQuery false dynamic "" str]}
      if {$sMxVersion >= 10.8} {
         foreach sDat [list 21 9 10 11 22 15 16 17] sPQ1 [list "" none none n none none n] sPQ2 [list frommeaning fromreviseaction fromcloneaction fromcardinality tomeaning toreviseaction tocloneaction tocardinality] {set aDat($sDat) [pPrintQuery $sPQ1 $sPQ2 "" ""]}
      } else {
   ...
   if {$bOverlay} {
      if {$sMxVersion >= 10.8} {
         pOverlay [list 4 6 9 10 11 12 15 16 17 18 21 22]
...
# Major/Minor Mod - ION - 10/1/2012
	  if {$bMm} {pOverlay [list 23]}
   }
}

# Procedure to process relationships
proc pProcessRelationship {} {
   global aCol aDat bAdd lsAttributePlan lsAttributeActual lsFromTypePlan lsFromTypeActual lsToTypePlan lsToTypeActual lsFromRelPlan lsFromRelActual lsToRelPlan lsToRelActual sHidden sHiddenActual sMxVersion bMm
   if {$bAdd} {
# Major/Minor Mod - ION - 10/1/2012
      if {$bMm} {
