...
Debug "\n\n emxProgramTriggerNotifyNewAssignees.tcl"
  ...
  foreach {sTypeProject sTypeTask sRelSubtask sRelAssignedTasks \
  ...
  set sTypeTask [lrange $sTypeTask 1 end]
  ...
  # RVW - R418 Cloud email issue setting variable to "true" by default
  #set sActivateSideDoor   [string tolower [string trim [mql get env 1]]]
   set sActivateSideDoor   "true"
...
Debug "OBJECT sAssigneeName = $sAssigneeName, task = $sName sEvent: $sEvent"
   ...
   set outStr [split [mql print bus $sObjectId select name type current  \
    ...
    physicalid from\[$sRelSubtask\] policy state dump |] |]
   ...
   set sPhysicalId [lindex $outStr 7]
   set sSummary [string tolower [lindex $outStr 8]]
   set sPolicy [lindex $outStr 9]
   set lStates [lrange $outStr 10 end]
  ...
  set sURLString  ""; 
		...
		#send 3DD URL
		set s3DDURLString  [lindex [array get env DASHBOARD_URL] 1]
		set sURLString [mql execute program emxMailUtil -method getStreamBaseURL ]

		if {$sURLString == ""} {
			set sURLString [lindex [array get env ENOVIA_URL] 1]
		}
		...
		set sNotificationText "Business Object: ${sTaskType} ${sTaskName} \n\n ${sNotificationText}"
		
		if { $bIsMember != -1 } {
			if {$sURLString != "" && $sPrjId != "" && $sEvent == "create"} {
				append  sNotificationText "\n\nENOVIA Project web app:  ${sURLString}?objectId=${sObjectId}"
			}
			if { $s3DDURLString != "" && $sEvent == "create" && $sSummary == "false" && $sTypeTask == $sTaskType } { # for revoke we dont't include the URL for TSK 
				append  sNotificationText "\n\nENOVIA Collaborative Tasks widget: ${s3DDURLString}#app:ENOTASK_AP/content:id=${sPhysicalId}"
			}
		}
    }
		...
		if { $sFound == 0 } {
			...
			if { "$sActivateSideDoor" == "true" } { 
			  ...
			  set mqlret [ catch {eval  $sCmd} outStr ]
				...
				set sCmd "mql send mail to \$sAssigneeName subject \$sSubject text \$sNotificationText"
				set mqlret [catch {eval $sCmd} outStr]
			}
		}
    }
