...
################################################################################
# Parses command and loads schema variables for any referenced property. Also
# creates link to the schema variable in the specified stack frame
#   sCommand - Command to be parsed
#   iLevel - variable level (0 = none, 1 = calling procedure, 2 = parent of
#            calling procedure, etc)
#
# This should be used by script utilities to load required schema variables
# instead of calling "LoadSchema" to load ALL schema variablea.
################################################################################

proc LoadSchemaVars { sCommand iLevel } {
    set lPropertyNames {}
    set lAdmins [list store vault attribute type relationship format group role person association policy wizard index]
    foreach sCommandWord $sCommand {
      foreach sWord [split $sCommandWord " ,"] {
        if {"[string index $sWord 0]" == "$"} {
            set sPropertyName [string range "$sWord" 1 end]
            if {"[string index $sPropertyName 0]" == "{" && "[string index $sPropertyName end]" == "}"} {
                set sPropertyName [string range "$sPropertyName" 1 end-1]
            }
            if {[lsearch $lAdmins [lindex [split $sPropertyName _] 0]] >= 0} {
                global "$sPropertyName"
                if {![info exists $sPropertyName]} {
                    set sCmd {mql print program eServiceSchemaVariableMapping.tcl select property\[$sPropertyName\].to dump}
                    set mqlret [ catch {eval $sCmd} outStr ]
                    set sPropertyValue [lrange [split $outStr] 1 end]
                    set "$sPropertyName" "$sPropertyValue"
                }
                lappend lPropertyNames "$sPropertyName"
            }
        }
      }
    }
    if {$iLevel > 0} {
        foreach sPropertyName $lPropertyNames {
            uplevel $iLevel global "$sPropertyName"
        }
    }
    return $lPropertyNames
}

################################################################################
# Loads schema variable for specified property (if not defined).  Also creates
# link to schema variable in the parent stack frame
#   sPropertyName - Property Name
#
# This should be used by script needing a few specific schema variables instead
# of calling "LoadSchema" to load ALL schema variablea.
################################################################################

proc LoadSchemaVar { sPropertyName } {
    global "$sPropertyName"
    if {![info exists $sPropertyName]} {
        set sCmd {mql print program eServiceSchemaVariableMapping.tcl select property\[$sPropertyName\].to dump}
        set mqlret [ catch {eval $sCmd} outStr ]
...
proc eServiceAddAdminObj {sOperation} {
    ...
    # Load Schema Vars (in parent context)
    LoadSchemaVars $sMQLCmd 2
    ...
    if {$bTableInstalled == 1} {
        ...
        switch $sOperation {
            "ADD TABLE" {
                ...
                if {[mql list table system "$sTableName"] != ""} {
