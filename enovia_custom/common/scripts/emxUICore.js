			...
			eval(strURL);
                ...
                if (strTarget != null) {
                	...
                	for(var i=0; i< params.length;i++){
                		...
                		if(param[0]==="targetLocation"){
                			strTarget = param[1];
                			if(param[1]==="slidein"){
                			getTopWindow().showSlideInDialog(strURL , "true");
                			break;
                			}else if(param[1]==="popup"){
...
emxUICore.getElementsByTagName = function (objRefNode, strTagName) {
    if (isIE || isMinFF3 || strTagName.indexOf(":") == -1) {
...
emxUICore.getXMLData = function _emxUICore_getXMLData(strURL, namespace) {
        ...
        ElapsedTimer.exit('getXMLData' + ' ' + objHTTP.responseText.length + ' chars');
        return objDOM;
};
...
emxUICore.debug.enumObject = function _emxUICore_debug_enumObject(objObject) {
        ...
        arrTemp.sort();
        this.write("<table border=\"0\"><tr><th>"+ emxUIConstants.STR_JS_AttOrMethod + "</th><th>"+ emxUIConstants.STR_JS_Type +"</th><th>"+ emxUIConstants.STR_JS_Value + "</th></tr>");
...
var registeredEvents = function(){
    ...
    return {
        ...
        registerEvent : function(HTMLControl, sEventName, fEventFunction, bCapture){
            lEvents.push(arguments);
        },

        // function to unregister all the events
        // this method is called on unload of the page
        unregisterAllEvents : function(){

        	try
        	{
	            var i = lEvents.length - 1;
	            // for each event cached
	            while(i >= 0){
	                // get event
	                var event = lEvents[i];
...
function showMyDesk()
{
	...
	if(getTopWindow().isfromIFWE){
		...
		if(getTopWindow().isTopFrameEnabled){
...
function submitWithCSRF(url,target){
...
function getWindowOpener(){
		if(getTopWindow().SnN && getTopWindow().SnN.FROM_SNN){
			resetFromSnN();
			if(getTopWindow().SnN.FRAME_TO_BE_REFRESHED != null){
...
function closeWindow(){
	...
	}else if(getTopWindow().slideInFrame)
	{
...
function getNavigatorWindowRef(){
	...
	if(currentTop.location.href.indexOf("emxNavigator.jsp") >=0){
		...
		_navigatorWinRef = contentFrame ? contentFrame.getTopWindow() : null;
	}
...
function updateURLParameter(url, param, paramVal){
    ...
    var position = additionalURL.indexOf("&");
    var subString;
    if(position == -1){
    	subString = additionalURL.substring(1,additionalURL.length);
    }else{
    	subString = additionalURL.substring(1,position);
    }
    additionalURL = additionalURL.replace(subString, paramVal);
    var finalURL = baseURL+param+additionalURL;
    return finalURL;
}
function isSnN () {

	if(this.defOptionsGlobal || getTopWindow().defOptionsGlobal){
		var widgetID = this.defOptionsGlobal ? this.defOptionsGlobal.socketName : getTopWindow().defOptionsGlobal.socketName;
		var isSnNEnabled = this.isSnNEnabled ? this.isSnNEnabled : getTopWindow().isSnNEnabled;
		if(widgetID != null && widgetID != "null" && isSnNEnabled){
			return true;
		}else{
			return false;
		}
...
function getPersistenceData(key, nameSpace ){
	...
	try {
		...
		return storage.get(nameSpace,key).value;
	} catch(e){
		return null;
	}
}
...
//add by fzq
function reloadAOReviewRouteTemplate()
{
    emxFormReloadField('ApprovalList');
}
function clearIssuePerson(IssueId,tableId)
{
	var sURL = "../common/SEMIssueRelated.jsp?mode=removePerson&IssueId="+IssueId;
   	sURL = encodeURI(sURL);
	var xmlHttp = getAjaxObj();	
	xmlHttp.onreadystatechange=function(){
		if(xmlHttp.readyState == 4){
			if(xmlHttp.status==200 || xmlHttp.status==304){							
				window.emxEditableTable.refreshRowByRowId(tableId);
			}
		} 
	};
...
//end
//add by wangyitao
function clearPerson(taskId,tableId)
{
	
	var sURL = "../common/SEMTaskManage.jsp?mode=removePerson&taskId="+taskId;
   	sURL = encodeURI(sURL);
	var xmlHttp = getAjaxObj();	
	xmlHttp.onreadystatechange=function(){
		if(xmlHttp.readyState == 4){
...
function clearTaskReviewer(taskId,tableId)
{
	
	var sURL = "../common/SEMTaskManage.jsp?mode=removeReviewer&taskId="+taskId;
   	sURL = encodeURI(sURL);
	var xmlHttp = getAjaxObj();	
	xmlHttp.onreadystatechange=function(){
		if(xmlHttp.readyState == 4){
...
//end
	...
	/**
	--------------add by wangyitao 2016-12-21  modify 2017-1-13 by fzq
	*/
	function update(object,sFieldName){
 
		   if (object.value=="") {
			if(sFieldName=="LSInformRouteTemplate"){
			  document.emxCreateForm.LSInformRouteTemplateDisplay.value="";
		      document.emxCreateForm.LSInformRouteTemplateDisplay.value="";
			  document.emxCreateForm.LSInformRouteTemplateDisplay.value="";
			}else{
		      document.emxCreateForm.LSRouteTemplateDisplay.value="";
		      document.emxCreateForm.LSRouteTemplateDisplay.value="";
			  document.emxCreateForm.LSRouteTemplateDisplay.value="";
			  }
		 // document.createDialog.txtdescription.value="";
			  ...
			  if(sFieldName=="LSInformRouteTemplate"){
				document.emxCreateForm.LSInformRouteTemplateDisplay.value=arr1[0];
		        document.emxCreateForm.LSInformRouteTemplateOID.value=arr1[1];
		        document.getElementById(sFieldName).value =  arr1[0];
			  }else{
		        document.emxCreateForm.LSRouteTemplateDisplay.value=arr1[0];
		        document.emxCreateForm.LSRouteTemplateOID.value=arr1[1];
		        document.getElementById(sFieldName).value =  arr1[0];
			  }
		  }
	}
	
	function updateJSP(object,sFieldName){
		 ...
		 // document.createDialog.txtdescription.value="";
		   ...
		   var arr1 = object.value.split("@");
		  document.frmMain.LSRouteTemplateDisplay.value=arr1[0];
		 document.frmMain.LSRouteTemplateOID.value=arr1[1];
	//chenshuang	 document.frmMain.CHRouteTemplate.value==arr1[0];
	     document.frmMain.LSRouteTemplate.value=arr1[0];
		  }
	}
	//add by ryan 2017-03-24
	function updateRouteJSP(object,sFieldName,LSRouteTemplateName){
	  if (object.value=="") {
	  document.getElementById(sFieldName).value="";
		document.getElementById(LSRouteTemplateName).value="";
	  }else{
		 var arr1 = object.value.split("@");
		document.getElementById(sFieldName).value=arr1[1];
		document.getElementById(LSRouteTemplateName).value=arr1[0];
		 }
	}
	//add end
	
	 function RefreshRouteTemplateList(){
		emxFormReloadField("LSRouteTemplate");		
	} 